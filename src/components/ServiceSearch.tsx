...
export function ServiceSearch({
  ...
  const [totalServices] = useState(initialCounts.categories.view_all || 0);
  const [error, setError] = useState<string | null>(null);
  const [counts, setCounts] = useState<ProcessedCounts>(
    processCountsData(initialCounts)
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const countsData: ApiCounts = await countsRes.json();

      setServices(searchData.services);
      setCounts(processCountsData(countsData));
    } catch (err) {
      setError(err instanceof Error ? err.message : "エラーが発生しました");
    } finally {
      setIsLoading(false);
    }
  }, [debouncedSearchQuery, selectedPlatform, selectedCategory, searchMode]);

  // 初回マウント時はスキップし、フィルター変更時のみAPIを呼ぶ
  useEffect(() => {
